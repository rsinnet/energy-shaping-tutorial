...
\begin{frame}[t]
  ...
  \only<3>{
    ...
    \begin{align*}
      ...
      L_{g} E_{c}(x, {\dot x}) &= \pd{E_{c}(x, {\dot x})}{{\dot x}} \frac{1}{M}\\
      &= \frac{K}{M} x.
    \end{align*}
  }
  ...
  \only<4>{
    ...
    \begin{align*}
      \mu(x) = \argmin_{u \in \R} \, & u^T u\\
      \mbox{s.t. } & \Aclf(x) \, u \leq \bclf(x)%\\
    \end{align*}
    where
    \begin{align*}
      \Aclf &=  2(E(x, {\dot x}) - E_{0}) \frac{K}{M},\\
      \bclf &= -2\epsilon(E(x, {\dot x}) - E_{0}).
    \end{align*} 
  }
